...
email_pass = environ.get("SMTP_PASSWORD")
...
def send_email(name, email, phone, message):
    ...
    with smtplib.SMTP("smtp.gmail.com") as connection:
        ...
        connection.sendmail(
            from_addr=my_email,
            to_addrs="mincho.tanev@abv.bg",
            msg="Subject:Blog Fan Email\n\n{}".format(message))
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if User.query.filter_by(email=form.email.data).first():
            flash("You have already used this email, log in.")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash("This email doesn't exist, please sign up.")
            return redirect(url_for("login"))
